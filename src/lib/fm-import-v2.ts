...
function parseStandings(rows: Record<string, unknown>[], module: "superleague" | "national", out: ParsedCompetitionsFile) {
  ...
  for (const r of rows) {
    ...
    out.standings.push({
      ...
      vp: (cols as Record<string, string | null>).vp ? toNum(r[(cols as Record<string, string>).vp]) : null,
      penalties: (cols as Record<string, string | null>).penalties ? toNum(r[(cols as Record<string, string>).penalties]) : null,
